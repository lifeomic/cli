...
async function search (type, query, options) {
  ...
  while (true) {
    ...
    if ( logResults ) {
      var resultBundle = data.entry.map(x => x.resource);
      print(resultBundle, options);
      loadedResultsCount = loadedResultsCount + resultBundle.length;
    } else {
      result.push(...data.entry.map(x => x.resource));
      loadedResultsCount = result.length;
    }

    if (loadedResultsCount < limit && data.link) {
