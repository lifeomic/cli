...
exports.builder = yargs => {
  yargs
    ...
    .option('force', {
      ...
      describe: 'Move each file after it is successfully uploaded. The value needs to the be the directory to which files should be moved.',
      type: 'string'
    });
};
    ...
    try {
      if (fileSize > MULTIPART_MIN_SIZE) {
        ...
        const response = await post(argv, FILES_URL, {
          ...
          datasetId: argv.datasetId,
          overwrite: argv.overwrite || argv.force,
          contentMD5: verifyStream.contentMD5
        });
