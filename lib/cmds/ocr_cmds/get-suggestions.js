...
const dateSuggestionResolver = dateString => {
  ...
  const timeStripped = asMoment
    ...
    .utc(false)
    .format(format);
  return moment(timeStripped, format);
};
...
const filterSuggestions = response => {
  ...
  potentialSuggestions.forEach(potentialSuggestion => {
    ...
    const extractedCondition = {
      ...
      ...conditionallyAddField('abatementDate', resolveValue(condition.abatementDate, dateSuggestionResolver)),
    ...
    if (
      extractedObservation.code && (
        extractedObservation.valueQuantity ||
        extractedObservation.valueString
      )) {
