...
const dateSuggestionResolver = dateString => {
  ...
  const timeStripped = asMoment
    .clone()
    .utc(false)
    .format(format);
  return moment(timeStripped, format).toISOString();
};
...
const getDate = (startDate, endDate, periodName, dateName) => {
  ...
  const returnValue = {};
  if (!endDate) {
    returnValue[dateName] = startDate;
  } else {
...
const conditionallyAddField = (fieldName, field) => {
  const returnValue = {};
  if (field) returnValue[fieldName] = field;
  return returnValue;
};
...
const filterSuggestions = response => {
  ...
  potentialSuggestions.forEach(potentialSuggestion => {
    ...
    if (extractedMedicationAdministration.medicationCodeableConcept) filteredSuggestions.push(extractedMedicationAdministration);
    if (extractedCondition.code) filteredSuggestions.push(extractedCondition);
    if (extractedObservation.code) filteredSuggestions.push(extractedObservation);
    if (extractedProcedure.code) filteredSuggestions.push(extractedProcedure);
  });
...
exports.builder = yargs => {
  yargs.positional('projectId', {
    ...
    s: {
      alias: 'suggestions',
      describe: 'Output raw suggestions instead of filtered FHIR',
      type: 'boolean'
    }
  });
};
