    ...
    for (file of files) {
      ...
      try {
        if (fileSize > MULTIPART_MIN_SIZE) {
          uploadResponse = await post(options, UPLOADS_URL, {
            name: fileName,
            datasetId: dataset,
            overwrite: options.overwrite
          });

          await multipartUpload(options, uploadResponse.data.uploadId, file, fileSize);
          console.log(chalk.green(`Upload complete: ${fileName}, ID: ${uploadResponse.data.id}`));
        } else {
          uploadResponse = await post(options, FILES_URL, {
            name: fileName,
            datasetId: dataset,
            overwrite: options.overwrite
          });

          await upload(uploadResponse.data.uploadUrl, file, fileSize);
          console.log(chalk.green(`Upload complete: ${fileName}, ID: ${uploadResponse.data.id}`));
        }
      } catch (error) {
