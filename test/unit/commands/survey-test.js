...
test.serial.cb('The export command can filter on a query', t => {
  getStub.onFirstCall().returns(questionnaire);
  getStub.onSecondCall().returns(responseOne);
  getStub.onThirdCall().returns(responseTwo);
  ...
  callback = () => {
    t.is(getStub.callCount, 3);
    t.is(getStub.getCall(0).args[1], '/v1/survey/projects/projectId/surveys/surveyId');
    t.is(getStub.getCall(1).args[1], '/v1/survey/projects/projectId/responses?surveyId=surveyId&pageSize=10&patientId=73d77961-a8b7-48e0-b76f-66e53485038b');
    t.is(getStub.getCall(2).args[1], 'testurl');
    t.is(plainPrintSpy.callCount, 1);
    t.is(plainPrintSpy.getCall(0).args[0], '"Patient ID","Date","Systolic blood pressure","Diastolic blood pressure","What country do you live in?"\n"73d77961-a8b7-48e0-b76f-66e53485038b","2020-06-04T14:56:14.402Z","120 millimeter of mercury","80 millimeter of mercury","United States of America"\n"73d77961-a8b7-48e0-b76f-66e53485038b","2020-06-04T14:56:14.402Z","120 millimeter of mercury","80 millimeter of mercury","United States of America"');
    t.end();
  ...
  yargs.command(exportResponses)
    .parse(`export-responses projectId surveyId --query 'patientId='73d77961-a8b7-48e0-b76f-66e53485038b`);
});

test.serial.cb('The export command can does not return data when query is not matched', t => {
  getStub.onFirstCall().returns(questionnaire);
  getStub.onSecondCall().returns({data: {items: [], links: {self: 'self'}}});

  callback = () => {
    t.is(getStub.callCount, 2);
    t.is(getStub.getCall(0).args[1], '/v1/survey/projects/projectId/surveys/surveyId');
    t.is(getStub.getCall(1).args[1], '/v1/survey/projects/projectId/responses?surveyId=surveyId&pageSize=10&patientId=123');
    t.is(plainPrintSpy.callCount, 1);
    t.is(plainPrintSpy.getCall(0).args[0], '"Patient ID","Date","Systolic blood pressure","Diastolic blood pressure","What country do you live in?"');
    t.end();
  };
  ...
  yargs.command(exportResponses)
    .parse(`export-responses projectId surveyId --query 'patientId=123'`);
});
